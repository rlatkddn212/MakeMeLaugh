...
namespace KZLib.KZDevelop
{
	public partial struct BigNum : IComparable<BigNum>,IEquatable<BigNum>
	{
		...
		private static (double,int,double,int) AdjustExponent(double _mantissa1,int _exponent1,double _mantissa2,int _exponent2)
		{
			...
			if(gap > 0)
			{
				_mantissa2 *= Math.Pow(10,+gap);

				return (_mantissa1,_exponent1,_mantissa2,_exponent1);
			}
			else if(gap < 0)
			{
				_mantissa1 *= Math.Pow(10,-gap);
		...
		/// <param name="_point">음수는 디폴트(단위가 없으면 0 / 있으면 2)이며 디폴트가 아니면 적힌 값으로 나온다.</param>
		/// <param name="_zero">소수가 0일 경우 false면 보여주지 않는다.</param>
		public string ToString(int _point = -1,bool _zero = true)
		{
			...
			var man = Mantissa*Math.Pow(10,Exponent-NumUnit.GetExpByUnit(unit));

			if(unit.IsEmpty())
			{
				_point = _point <= -1 ? 0 : _point;
			}
			else
			{
				_point = _point <= -1 ? 2 : _point;
			}
