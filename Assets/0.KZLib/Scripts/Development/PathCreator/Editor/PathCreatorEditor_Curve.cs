	...
	public partial class PathCreatorEditor : OdinEditor
	{
			...
			if(_event.button == 0)
			{
				switch(_event.type)
				{
					...
					case EventType.MouseDrag when m_DragHandleIndex != -1:
					{
						...
						var localPosition = Tools.InverseTransformPoint(GetMouseWorldPosition(m_Creator.PathSpaceType),m_Creator.transform,m_Creator.PathSpaceType);
		...
		private void DrawLineInCurve(Vector3[] _handleArray)
		{
			...
			for(var i=0;i<length;i++)
			{
				...
				Handles.DrawDottedLine(_handleArray[i*3+2],_handleArray[Tools.LoopClamp(i*3+3,_handleArray.Length)],5.0f);
			}
