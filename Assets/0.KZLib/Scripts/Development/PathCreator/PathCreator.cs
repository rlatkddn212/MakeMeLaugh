...
namespace KZLib.KZDevelop
{
	...
	public partial class PathCreator : BaseComponent
	{
		...
		[VerticalGroup("2",Order = 2),ShowInInspector,LabelText("$ResolutionLabel"),MinValue("$MinResolution")]
		...
		public Vector3[] PointArray
		{
			get
			{
				if(m_PointArray == null || m_IsDirty)
				{
					if(IsCurveMode)
					{
						GetCurvePointArray();
					...
					m_PathLength = Tools.GetTotalDistance(m_PointArray);
