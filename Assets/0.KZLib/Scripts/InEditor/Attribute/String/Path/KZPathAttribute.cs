...
namespace KZLib.KZAttribute
{
	...
	{
		public bool AddOpenButton { get; private set; }
		public bool AddChangeButton { get; private set; }
	...
	public abstract class KZPathAttributeDrawer<TAttribute> : KZAttributeDrawer<TAttribute,string> where TAttribute : KZPathAttribute
	{
		...
		private Rect OnClickToChangePath(Rect _rect,bool _)
		{
			return DrawButton(_rect,SdfIconType.ArrowRepeat,true,()=>
			{
				...
				{
					// 내부에 있는 경우
					if(!FileTools.IsIncludeAssetsHeader(dataPath))
					{
						Tools.DisplayError(string.Format("{0}가 Assets 폴더 안에 있지 않습니다. 다시 확인해 주세요.",dataPath));
					}

					ValueEntry.SmartValue = FileTools.RemoveAssetsHeader(dataPath);
				}
				else
				{
					// 외부에 있는 경우
		...
		protected Rect DrawParentFolderOpenButton(Rect _rect,bool _isValid)
		{
			...
			{
				FileTools.OpenFolder(FileTools.GetParentPath(ValueEntry.SmartValue));
			});
		}
	}
#endif
