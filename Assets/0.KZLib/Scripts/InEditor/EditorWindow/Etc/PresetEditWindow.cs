	...
	public class PresetEditWindow : OdinEditorWindow
	{
		...
		private ShowType CurrentType
		{
			...
			set
			{
				...
				m_PresetList.AddRange(Tools.LoadAllAssetsInEditor<Preset>("t:preset"));
		...
		[HorizontalGroup("Preset",Order = PRESET_ORDER),ShowInInspector,HideLabel,DisplayAsString,HideIf("m_IsExistPreset")]
		private string Exist_Display { get => "존재하지 않습니다."; set { } }

		[HorizontalGroup("Preset",Order = PRESET_ORDER),SerializeField,LabelText("프리셋 리스트"),ListDrawerSettings(ShowFoldout = false),ShowIf("m_IsExistPreset"),Searchable]
