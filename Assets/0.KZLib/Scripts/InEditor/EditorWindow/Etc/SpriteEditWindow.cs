...
namespace KZLib.KZWindow
{
	public class SpriteEditWindow : OdinEditorWindow
	{
		...
		{
			m_SpritePath = FileTools.GetFilePathInPanel("위치를 수정 합니다.",".png");
		}

		[HorizontalGroup("버튼 그룹/0"),Button("이미지 변환하기",ButtonSizes.Large),EnableIf("$IsExist")]
		private void OnSetImage()
		{
			var fileData = FileTools.ReadFile(m_SpritePath);
			...
			try
			{
				if(texture.LoadImage(fileData))
				{
					...
					if(flag)
					{
						...
						FileTools.WriteFile(string.Concat(FileTools.GetPathWithoutExtension(m_SpritePath),"_Convert.png"),texture);
						Tools.DisplayInfo("이미지 변경 완료");
					}
				}
				else
				{
			...
			{
				Tools.DisplayError(string.Format("이미지 변경 실패 [{0}]",_ex.Message));
			}
		}
