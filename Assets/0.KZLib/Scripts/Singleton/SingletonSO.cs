...
public abstract class SingletonSO<TObject> : SerializedScriptableObject where TObject : ScriptableObject
{
	...
	protected static void SaveAsset(string _filePath)
	{
		...
		FileTools.CreateFolder(filePath);

		Log.InGame.I(filePath);
		Log.InGame.I(m_Instance);
		Log.InGame.I(FileTools.GetAssetsPath(filePath));

		AssetDatabase.CreateAsset(m_Instance,FileTools.GetAssetsPath(filePath));
	...
	public static TObject In
	{
		get
		{
			if(!m_Instance)
			{
				...
				var path = FileTools.PathCombine(string.Format(Global.IN_SIDE_SO_PATH,"Settings"),typeName);
	...
	public static TObject In
	{
		get
		{
			if(!m_Instance)
			{
				var typeName = typeof(TObject).Name;
#if UNITY_EDITOR
				var path = FileTools.PathCombine(string.Format(Global.OUT_SIDE_SO_PATH,"Settings"),typeName);

				m_Instance = AssetDatabase.LoadAssetAtPath<TObject>(FileTools.GetAssetsPath(string.Format("{0}.asset",path)));
