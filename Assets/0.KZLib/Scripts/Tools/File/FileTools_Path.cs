...
{
	private const string ASSETS_HEADER = "Assets";
	...
	public static string GetAssetsPath(string _path)
	{
		...
		if(_path.Contains(ASSETS_HEADER))
		{
			_path = RemoveAssetsHeader(_path);
		}

		return PathCombine(ASSETS_HEADER,_path);
	}
	...
	{
		return _path.Contains(ASSETS_HEADER);
	}

	public static bool IsStartWithAssetsHeader(string _path)
	{
		return _path.StartsWith(ASSETS_HEADER);
	}
	...
	/// <summary>
	/// _header를 제거한 파일 경로를 반환함.
	/// </summary>
	public static string RemoveHeaderDirectory(string _path,string _header)
	{
		return _path[(_path.IndexOf(_header)+_header.Length+1)..];
	}
	...
	{
		return RemoveHeaderDirectory(_path,ASSETS_HEADER);
	}
}